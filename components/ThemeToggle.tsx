...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    const stored = (localStorage.getItem("orion-theme") as Theme | null) ?? "dark";
    setTheme(stored);
  ...
  return (
    ...
    >
      {mounted && (
        <>
          {/* destination hint — the opposite-mode icon shown on the empty side of the track */}
          <MoonIcon
          ...
          {/* twinkling stars in the dark portion */}
          {isDark && (
            <>
              ...
              <span className="absolute top-3 left-[3.2rem] w-[2px] h-[2px] rounded-full bg-white opacity-70 animate-twinkle"
